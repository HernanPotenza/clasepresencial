...
def crear_tablero():
    ...
    tablero["tiempo_ultimo_destape"] = 0 #pygame.time.get_ticks()
...
def generar_lista_tarjetas()->list:
    ...
    for x in range(0, CANTIDAD_TARJETAS_H * ANCHO_TARJETA, ANCHO_TARJETA):
        for y in range(0, CANTIDAD_TARJETAS_V * ALTO_TARJETA, ALTO_TARJETA):
            nueva_tarjeta = tarjeta.crear_tarjeta(lista_id[indice], lista_id[indice], "00.png", x, y)
...
def dibujar_tablero(tablero: dict, pantalla_juego: pygame.Surface):
    ...
    #     print(tablero["tarjetas"][i]["rectangulo"].x)
    #     pantalla_juego.blit(tablero["tarjetas"][i]["superficie"], (tablero["tarjetas"][i]["rectangulo"].x,tablero["tarjetas"][i]["rectangulo"].y))
    for i in range(CANTIDAD_TARJETAS_H*CANTIDAD_TARJETAS_V):
        if tablero["tarjetas"][i]["visible"] == False:
            pantalla_juego.blit(tablero["tarjetas"][i]["superficie_escondida"], (tablero["tarjetas"][i]["rectangulo"].x,tablero["tarjetas"][i]["rectangulo"].y))
        else:   
            pass
    # COMPLETAR
